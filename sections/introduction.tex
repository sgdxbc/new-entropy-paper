...
\sgd{Reworked the rest of this section from here.}
This placement metadata must remain reliable throughout each block's lifetime to support retrieval and repair.
Some DSNs maintain placement state on chain through smart contracts, whereas others derive placement groups algorithmically through deterministic rules or protocols.
However, placement is particularly challenging under Byzantine behavior: malicious nodes can ignore incentives~\cite{bar}, collude within a placement group, and discard redundancy simultaneously~\cite{ipfs-attack,bittorrent-attack}, creating correlated failures that violate independent-failure assumptions.
...
These DSNs suffer from limited and non-scalable storage volume~\cite{permacoin,retricoin,arweave,walrus,bft-dsn,swarm}, on-chain footprint~\cite{filecoin,sia,shelby} and/or Byzantine fault tolerance~\cite{filecoin,sia,shelby,permacoin,retricoin,arweave,walrus,bft-dsn}.
...
In this work, we propose a novel design principle of DSNs: a node-centric approach that allows defense to ``track'' Byzantine nodes across placement groups.
Unlike prior data-centric approaches, in which placement groups are paid for persisting specific data, our approach pays nodes directly for placement-group participation, regardless of which groups they join.
...
Under this node-centric model, the behavior of rational nodes is insensitive to Byzantine distribution because their profits remain unaffected by where Byzantine nodes cluster.
